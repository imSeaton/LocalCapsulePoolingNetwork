...
class Trainer(object):
    ...
    def addModel(self):
        if self.p.model == 'CapsulePoolingGraphNetwork':
            model = CapsulePoolingGraphNetwork(
                    ...
                    global_pool_mode=self.p.global_pool_mode
                )
        else:
    ...
    def run_epoch(self, loader):
        ...
        for data in loader:
            ...
            if self.p.global_pool_mode == 'ReadoutPooling':
                loss = F.nll_loss(out, ground_truth.view(-1))
            elif self.p.global_pool_mode == 'HierarchicalCapsulePooling':
                loss = margin_loss(out, data.y.view(-1))
            else:
                print("wrong global_pool_mode!!")
                return
            loss.backward()
    ...
    def run(self):
        ...
        val_acc_std = np.round(np.std(val_accs), 4)
        test_acc_std = np.round(np.std(test_accs), 4)
        print(f"seed {self.seed}  val_acc_mean {val_acc_mean:.3f}±{val_acc_std:.3f}     test_acc_mean {test_acc_mean:.3f} ± {test_acc_std:.3f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data', dest='dataset', default='DD', type=str, help='Dataset to use')
    ...
    parser.add_argument('-global_pool_mode', dest='global_pool_mode',
                        choices=['HierarchicalCapsulePooling', 'ReadoutPooling'],
                        default='ReadoutPooling', help='Global Pooling Mode')
    ...
    seeds = [8971, 85688, 9467, 32830, 28689]
    counter = 0
